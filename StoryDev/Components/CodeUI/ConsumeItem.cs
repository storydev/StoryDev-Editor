...
{
    partial class ConsumeItem : UserControl, ICodeComponent
    {
        public ConsumeItem()
        {
            InitializeComponent();
        ...
        {
            var result = code.Substring("ConsumeItem(".Length, code.LastIndexOf(')') - "ConsumeItem(".Length);
        ...
        {
            return string.Format("ConsumeItem({0})", Globals.Items[cmbItems.SelectedIndex].ID);
        }
